...
SELECT
    ...
    ROUND(COALESCE(sp.overall_ctr, 0), 3) AS ctr,
    ...
    ROUND(SAFE_DIVIDE(COALESCE(rv.total_orders, 0),
          COALESCE(se.total_sessions, 1)), 3)  AS session_cvr,
...
WITH monthly_spend AS (
    ...
    GROUP BY c.channel, c.campaign_type,year_month
),
monthly_revenue AS (
    SELECT
        con.channel,
...
LEFT JOIN monthly_revenue mr
    ...
    AND ms.campaign_type = mr.campaign_type  
    AND ms.year_month = mr.year_month
ORDER BY ms.year_month, ms.channel, ms.campaign_type;
...
SELECT
    ...
    ROUND(AVG(session_cvr), 3)      AS avg_cvr,
