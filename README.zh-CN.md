...
本项目选择使用 Python 生成模拟数据，原因如下：

- **完整管线控制**：模拟数据让整个项目得以覆盖完整的数据分析流程——从原始事件生成、ETL 清洗、数据建模、SQL Views 到 Dashboard 可视化——不受公开数据集字段限制的制约。
- **贴近真实业务场景**：各项基准参数（CTR、CVR、平均订单金额、季节性系数）均以业界公开的数字营销基准为依据（例如：Google Ads 行业平均 CTR 4–6%；电子邮件营销 CVR 3–6%；Q4 季节性涨幅 +20–30%）。模拟数据的分布规律与真实市场高度吻合，包括 Email 低成本高 ROAS 的特性，以及 Display 广告 CPA 偏高的典型表现。
- **可重现性与可审计性**：使用 `random.seed(42)` 确保任何分析人员均可重新生成完全相同的数据集，并验证所有 SQL 输出结果——这是供技术面试官审阅的 Portfolio 项目所必须具备的条件。
- **跨渠道可比性**：在五个渠道（Google Ads、Facebook Ads、Email、Organic、Direct）中以一致的 Schema 构建数据，使跨渠道 ROI 比较分析成为可能；而这种分析在真实的碎片化数据源中极难完整实现。

> **说明：** 所有模拟参数（CTR、CVR、订单金额、Q4 季节性系数）已记录于 [模拟参数](#数据仿真参数) 一节，并完整收录于 [`docs/data_dictionary.zh-CN.md`](./docs/data_dictionary.zh-CN.md)。所有分析结论均在本模拟情境的框架内作出解读。
---
