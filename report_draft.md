...
CTR 分桶分析揭示了一個重要洞察：CTR 與 CVR 之間**並不呈線性正相關**——但這個規律在渠道篩選下才最為明顯。

⚠️ 重要前提： Dashboard Page 2 的 KPI 數值（Avg CTR 3.26%、Avg CVR 4.46%）採用的是各 Campaign 的**簡單平均（Simple Average）**，而非加權平均，如右上角備注所示。All Channel 的整體 CTR Bucket 圖因 Google Ads 高訂單量主導，CVR 趨勢線看似持續上升；**以下「高 CTR ≠ 高 CVR」的結論需在 Email 渠道篩選下觀察才最清晰。**
...
在 Email 渠道篩選下，Dashboard 的 Revenue & CVR by CTR Range 圖明確呈現 CVR 在 CTR 3–4% 達峰後下降。這說明 Email 的高 CTR 素材（CTR ≥ 4–5%）反而吸引了較多好奇點擊但購買意圖偏低的用戶，廣告吸引力與產品匹配度之間存在落差。相比之下，All Channel 視圖因 Google Ads 的高訂單量稀釋了 Email 的高 CVR 訊號，需拆分渠道觀察才能得出正確結論。
